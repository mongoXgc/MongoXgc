 ...
 const fractureImageNames = await fracture_Database.find({ UserId:userId });
    ...
    const fractureSignedURLs = await Promise.all(
      fractureImageNames.map(async (image) => {
        ...
        return {signedURL, comment: image.comment, output: image.output, uniqueID : image._id};
      })
    );

// -----------------------------

    const anemiaFileNames = await anemia_Database.find({ UserId:userId });
...
const heartFileNames = await heartAttack_Database.find({ UserId:userId });
...
const tumorSignedURLs = await Promise.all(
 ...
 res.render('collections.ejs', { fractureSignedURLs , anemiaSignedURLs, heartSignedURLs, tumorSignedURLs});
    ...
    //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~    
          ...
          if(finalData.data.output.includes('No fracture is detected')){
            ...
            const userName = await userProfile_Database.findById(data.UserId);
              res.render('resultOutput.ejs',{ name:userName.name, resultImage: resultImage, resultString: resultString, portName: port, ipAddress: ip});
          }
...
//Manual Entries -----------------------------------------------------
  ...
  if(selectedOption == 'anemia'){
    const {anemia_hemoglobin , anemia_rbc ,anemia_hematocrit, anemia_wbc, anemia_neutrophils, anemia_lymphocytes, anemia_monocytes, anemia_eosinophils, anemia_platelet, anemia_mcv,anemia_mch,anemia_mchc} = req.body;
     
    const Data = await axios.get(`${ip}${port}/anemia-manual-Processing-result`, {params:{anemia_hemoglobin , anemia_rbc ,anemia_hematocrit, anemia_wbc, anemia_neutrophils, anemia_lymphocytes, anemia_monocytes, anemia_eosinophils, anemia_platelet, anemia_mcv,anemia_mch,anemia_mchc}});
    console.log("Manual Data mil gaya: ", Data.data);
          ...
          if(Data.data.includes('The patient is Normal')){
            ...
            res.render('resultOutput.ejs',{ name: "null",resultImage: resultImage, resultString: resultString, portName: port, ipAddress: ip});
          }
    ...
    const {heart_gender, heart_age, heart_chestpain, heart_restbp, heart_chol, heart_fbp, heart_restecg, heart_thalach, heart_exang, heart_oldpeak, heart_slope, heart_ca, heart_thal} = req.body;
  
  
    const Data = await axios.get(`${ip}${port}/heartAttack-manual-Processing-result`, {params:{heart_gender, heart_age, heart_chestpain, heart_restbp, heart_chol, heart_fbp, heart_restecg, heart_thalach, heart_exang, heart_oldpeak, heart_slope, heart_ca, heart_thal}});
    console.log("Manual Data mil gaya: ", Data.data);
          let resultImage = 'kuch nhi';
          let resultString = '';
